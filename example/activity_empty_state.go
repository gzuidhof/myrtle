...
func ActivityEmptyStateEmailWithTheme(selectedTheme theme.Theme) (*myrtle.Email, error) {
	...
	return myrtle.NewBuilder(
		...
		WithHeader(commonHeaderGroup("Myrtle Ops", selectedTheme)).
		...
		AddCallout(myrtle.ToneWarning, "Stay prepared", "Create escalation policies before the next incident.", myrtle.CalloutStyle(myrtle.CalloutVariantOutline)).
